...
if not SSO_URL: raise RuntimeError("SSO_URL env not set")
INTERNAL_API_KEY = os.environ.get("INTERNAL_API_KEY")
if not INTERNAL_API_KEY: raise RuntimeError("INTERNAL_API_KEY env not set")
DEBUG = os.getenv("DEBUG", "").lower() == "true"

def validate_session(req) -> dict:
    session_id = req.cookies.get("session_id")
    if DEBUG:
        return {"cid": "1234567", "roles": ["administrator"]}
    if not session_id:
